        ...
        <div className="flex flex-col gap-2 w-full">
            ...
            <div className="flex justify-between w-full text-sm text-muted-foreground">
                <div className="flex flex-col items-start">
                    <strong className="text-black">{market.optionA}</strong>
                    <span>{Math.floor(parseInt(toEther(sharesBalance?.optionAShares)))}</span>
                </div>
                <div className="flex flex-col items-end">
                    <strong className="text-black">{market.optionB}</strong>
                    <span>{Math.floor(parseInt(toEther(sharesBalance?.optionBShares)))}</span>
                </div>
            </div>
            {(winnings.A > 0 || winnings.B > 0) && (
                <div className="flex flex-col gap-1 w-full">
                    ...
                    <div className="flex justify-between w-full gap-2">
                        <Badge variant="secondary" className="w-full">{market.optionA}: {displayWinningsA} shares</Badge>
                        <Badge variant="secondary" className="w-full">{market.optionB}: {displayWinningsB} shares</Badge>
                    </div>
                </div>
            )}
        </div>
